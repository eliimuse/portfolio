...
if (pathEl) {
  skillCategories.forEach((cat, i) => {
    ...
    dot.addEventListener("click", () => renderSkillPanel(i));
    pathEl.appendChild(dot);
  });
}

const dotEls = pathEl ? [...pathEl.children] : [];
let lastActiveIndex = -1;

function renderSkillPanel(index) {
  if (!skillCategories[index] || !eyebrowEl || !titleEl || !descEl || !tagsEl) return;
